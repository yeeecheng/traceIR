...
class DaCLIP(nn.Module):
    ...
    def cosine_similarity(self, image, text):

        image_features, degra_features = self.encode_image(image, control= True)
        # degra_features = torch.cat([degra_features, torch.zeros(1, 256).to("cuda:1")], dim=1).to("cuda:1")
